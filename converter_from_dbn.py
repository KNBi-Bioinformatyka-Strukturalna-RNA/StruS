    ...
    for position, char in enumerate(structure, start=1):
        ...
        if char in CLOSE_TO_OPEN:
            ...
            if not stacks[opening]:
                raise ValueError(f"Invalid dot-bracket structure: unmatched '{char}' at position {position}")
            left = stacks[opening].pop()
            pairs.append((left, position, opening))
            continue
        raise ValueError(f"Invalid dot-bracket structure: unsupported character '{char}' at position {position}")

    unmatched = []
    for opening, stack in stacks.items():
        unmatched.extend((opening, position) for position in stack)
    if unmatched:
        details = ", ".join(f"'{opening}' at {position}" for opening, position in sorted(unmatched, key=lambda x: x[1]))
        raise ValueError(f"Invalid dot-bracket structure: unmatched opening brackets: {details}")
...
def find_stems(pairs):
    ...
    for left, right, bracket in pairs:
        ...
        left_pos = left
        right_pos = right
        while (left_pos + 1, right_pos - 1, bracket) in pair_set:
            left_pos += 1
            right_pos -= 1
            next_pair = (left_pos, right_pos, bracket)
            current.append(next_pair)
            used.add(next_pair)
        stems.append({"left_first": current[0][0], "left_last": current[-1][0], "right_first": current[-1][1], "right_last": current[0][1], "bracket": bracket})
    stems.sort(key=lambda stem: (stem["left_first"], stem["right_last"]))
...
def stem_contains(parent, child):
    ...
    return parent["left_first"] < child["left_first"] and child["right_last"] < parent["right_last"]
...
def assign_loop_children(stems):
    for i, child in enumerate(stems):
        ...
        for j, parent in enumerate(stems):
            ...
            nested = parent_start < child_start and child_end < parent_end
            crossing = parent_start < child_start < parent_end < child_end or child_start < parent_start < child_end < parent_end
            if nested or crossing:
                candidates.append((abs(parent_start - child_start), parent_end - parent_start, j))
...
def add_element(elements, name, bear_type, strands, branching=False, extra=None):
    ...
    element = {"id": element_id, "name": name, "bear_type": bear_type, "branching": branching, "parent": None, "children": [], "strands": strands}
    if extra:
...
def classify_loop_for_stem(stem_index, stems, sequence, structure):
    ...
    if len(children) == 1 and not crossing_children:
        ...
        right_end = loop_end
        left_length = max(0, left_end - left_start + 1)
        right_length = max(0, right_end - right_start + 1)
        if left_length == 0 and right_length == 0:
            return {"type": "Stack", "branching": False, "children": children, "segments": []}
        if left_length > 0 and right_length > 0:
            return {"type": "InternalLoop", "branching": False, "children": children, "segments": [("LEFT", left_start, left_end), ("RIGHT", right_start, right_end)]}
        if left_length > 0:
            return {"type": "BulgeLeft", "branching": False, "children": children, "segments": [("LEFT", left_start, left_end)]}
        return {"type": "BulgeRight", "branching": False, "children": children, "segments": [("RIGHT", right_start, right_end)]}
    ...
    for child_index in children:
        ...
        if child_left > loop_end:
            continue
        segment_end = min(child_left - 1, loop_end)
        if current <= segment_end:
            segments.append((current, segment_end))
        current = max(current, child_right + 1)
        if current > loop_end:
            break

    if current <= loop_end:
        segments.append((current, loop_end))

    return {"type": "Junction", "branching": True, "children": children, "segments": segments}

def element_start_position(element):
    positions = [strand["first"] for strand in element["strands"] if strand is not None]
    if not positions:
        return float("inf")
...
def chain_top_level(elements):
    top_level = [element for element in elements if element["parent"] is None]
    top_level.sort(key=element_start_position)
    for previous, current in zip(top_level, top_level[1:]):
        link(elements, previous["id"], current["id"])

    return elements

def find_unpaired_regions(structure, paired_positions):
    regions = []
    start = None
    for position in range(1, len(structure) + 1):
    ...
    if start is not None:
        regions.append((start, len(structure)))

    return regions

def build_secondary_structure_graph(sequence, structure):
    pairs = get_pairs(structure)
    elements = []
    if not pairs:
        if not sequence:
    ...
    for index in range(len(stems)):
        motif = classify_loop_for_stem(index, stems, sequence, structure)
        motif_type = motif["type"]

        if motif_type == "Stack":
            child_index = motif["children"][0]
            link(elements, stem_element_id[index], stem_element_id[child_index])
            continue
        ...
        if motif_type == "Hairpin":
            counters["Hairpin"] += 1
            loop_id = add_element(elements, f"Hairpin {counters['Hairpin']}", "LOOP", motif["strands"])
            link(elements, stem_element_id[index], loop_id)
            continue
    ...
    paired_positions = {position for left, right, _ in pairs for position in (left, right)}
    regions = find_unpaired_regions(structure, paired_positions)
    single_id = 1
...
def find_element_at_position(elements, position):
    candidates = []
    for element in elements:
        for strand in element.get("strands", []):
            if strand is None:
                continue
            first = strand["first"]
            last = strand["last"]
            if first <= position <= last:
                span = last - first + 1
                is_stem = element["bear_type"] in ("Stem", "STEM_branch")
                candidates.append((is_stem, span, first, element["id"]))
...
def find_pseudoknot_stems(original_structure):
    pairs = get_pairs(original_structure)
    pseudoknot_pairs = [pair for pair in pairs if pair[2] != "("]
    if not pseudoknot_pairs:
        return []

    return find_stems(pseudoknot_pairs)

def add_pseudoknots(elements, sequence, original_structure, normal_stem_count):
    pk_stems = find_pseudoknot_stems(original_structure)
    if not pk_stems:
        return elements

    pk_stems.sort(key=lambda stem: (stem["left_first"], stem["right_last"]))
    for pk_index, stem in enumerate(pk_stems, start=1):
        left_position = stem["left_first"]
        right_position = stem["right_last"]
        left_element_id = find_element_at_position(elements, left_position)
        right_element_id = find_element_at_position(elements, right_position)
        source_elements = []
        if left_element_id is not None:
            source_elements.append(left_element_id)
        if right_element_id is not None and right_element_id not in source_elements:
            source_elements.append(right_element_id)
...
def build_structure_graph(sequence, structure, remove_pseudoknots_flag=False):
    secondary_structure = remove_pseudoknots(structure)
    graph = build_secondary_structure_graph(sequence, secondary_structure)
    if remove_pseudoknots_flag:
        return graph

    pk_stems = find_pseudoknot_stems(structure)
    if not pk_stems:
        return graph

    normal_stem_count = sum(1 for element in graph if element["bear_type"] in ("Stem", "STEM_branch"))
    graph = add_pseudoknots(graph, sequence, structure, normal_stem_count)
    return graph
