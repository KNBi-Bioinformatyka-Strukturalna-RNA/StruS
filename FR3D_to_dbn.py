...
parser = argparse.ArgumentParser(
    description="Convert FR3D cWW base-pair annotations to extended DBN format"
)
parser.add_argument(
    "fr3d",
    help="Input FR3D annotation file"
)
parser.add_argument(
    "pdb",
    help="Input PDB file"
...
parser.add_argument(
    "-o", "--output",
    default="structure.dbn",
    help="Output DBN file"
)
...
def read_pdb_sequence(path):
    ...
    if not residues:
        raise ValueError(
            f"No RNA residues (A, C, G, U) found in PDB file '{path}'"
        )
...
def read_fr3d(path, residues):
    ...
    for index, (chain, resseq, icode, resname) in enumerate(
        residues, start=1
    ):
        key = (chain, resseq)
        ...
        if key in residue_positions:
            raise ValueError(
                f"Ambiguous PDB residue numbering: chain {chain}, "
                f"residue {resseq} occurs more than once"
            )
    ...
    try:
        with path.open() as f:
            for line_number, line in enumerate(f, start=1):
                ...
                if len(fields) < 3:
                    raise ValueError(
                        f"Invalid FR3D line {line_number}: expected at least 3 fields"
                    )
                ...
                try:
                    ...
                    raise ValueError(
                        f"Invalid residue identifier in FR3D line "
                        f"{line_number}: {line}"
                    )
                ...
                if key1 not in residue_positions:
                    raise ValueError(
                        f"FR3D residue {residue1} was not found in PDB"
                    )

                if key2 not in residue_positions:
                    raise ValueError(
                        f"FR3D residue {residue2} was not found in PDB"
                    )
                ...
                if pos1 == pos2:
                    raise ValueError(
                        f"Self-pair detected at position {pos1}"
                    )
...
def assign_bracket_types(pairs):
    """
    Assign bracket types to base pairs.

    Two pairs that cross each other cannot use the same bracket type.

    Nested pairs may use the same bracket type, while crossing pairs
    receive different bracket types.
    ...
    cross and therefore need different bracket types.
    """
    ...
    for pair in sorted(pairs):
        ...
        for bracket_type in range(len(BRACKET_TYPES)):
            ...
            raise ValueError(
                f"Too many mutually crossing base pairs. "
                f"Maximum supported bracket types: {len(BRACKET_TYPES)}"
            )
...
def build_dbn(residues, pairs):
    ...
    for left, right in pairs:
        ...
        if structure[left - 1] != ".":
            raise ValueError(
                f"Position {left} is involved in multiple base pairs"
            )

        if structure[right - 1] != ".":
            raise ValueError(
                f"Position {right} is involved in multiple base pairs"
            )
...
def main():
    try:
        ...
        sequence, structure, assignments = build_dbn(
            residues,
            pairs
        )
